...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		soundRecordIntent.putExtra(SoundRecordActivity.EXTRA_OUTPUT, audioFile.getPath());
	...
	private void launchCameraIntent() {
		...
		// Fixes NPE as described on the forum
		// https://class.coursera.org/android-001/forum/thread?thread_id=10593#post-38018 
		fragment.imagePath = imageFileUri;	
		
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		mediaStoreIntent.putExtra(MediaStore.EXTRA_OUTPUT, imageFileUri);
	...
	private void launchVideoCameraIntent() {
		...
		// Hint: use standard Intent from MediaStore class
		// See: http://developer.android.com/reference/android/provider/MediaStore.html
		Intent mediaStoreIntent = new Intent(MediaStore.ACTION_VIDEO_CAPTURE);
		...
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		mediaStoreIntent.putExtra(MediaStore.EXTRA_OUTPUT, videoFile.getPath());
		...
		mediaStoreIntent.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, 1);

		
		// TODO - Start a new activity for result, using the new intent and the request
		// code CAMERA_VIDEO_REQUEST
		startActivityForResult(mediaStoreIntent, CAMERA_VIDEO_REQUEST);
